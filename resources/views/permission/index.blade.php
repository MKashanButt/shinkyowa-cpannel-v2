@php
    $sno = ($permissions->currentPage() - 1) * $permissions->perPage();
@endphp

<x-app-layout>
    <section>
        <x-breadcrumbs :page="'Settings'" :subpage="'Permissions'" />
        <x-header>
            {{ __('Permissions') }}
                ...
                <tbody class="bg-white divide-y divide-[#e3e3e0]">
                    @foreach ($permissions as $key => $data)
                        <tr>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-xs">{{ str_replace('_', ' ', $data['name']) }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-xs">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-xs">
                                <div class="flex gap-4">
                                    <a href="{{ route('permission.edit', $data) }}">
                                        <x-primary-button>Edit</x-primary-button>
                                    </a>
                                    <form action="{{ route('permission.destroy', $data) }}" method="POST"
                                        x-data="{ open: false }">
                                        @method('DELETE')
                                        @csrf
