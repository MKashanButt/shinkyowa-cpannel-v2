@php
    $sno = ($inquiries->currentPage() - 1) * $inquiries->perPage();
@endphp

<x-app-layout>
    <section>
        <x-breadcrumbs :page="'Website'" :subpage="'Inquiries'" />
        <x-header>
            {{ __('Inquiries') }}
        </x-header>
                ...
                <tbody class="bg-white divide-y divide-[#e3e3e0]">
                    @foreach ($inquiries as $key => $data)
                        <tr>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-xs">{{ $data['country']->name }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-xs">{{ $data['message'] }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-xs">{{ $data['ip'] }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-xs">
                                <div class="flex gap-4">
                                    <a href="{{ route('inquiry.show', $data) }}">
                                        <x-primary-button>View</x-primary-button>
                                    </a>
                                    <form action="{{ route('inquiry.destroy', $data) }}" method="POST"
                                        x-data="{ open: false }">
                                        @method('DELETE')
                                        @csrf
